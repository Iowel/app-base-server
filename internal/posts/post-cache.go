...
func (cache *redisCache) Set(key string, value *Post) {
...
func (cache *redisCache) Get(key string) *Post {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (cache *redisCache) GetByUserID(userID string) []*Post {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	for {
		keys, nextPoint, err := client.Scan(ctx, point, "post:*", 100).Result()
		if err != nil {
		...
		for _, key := range keys {
			...
			if err := json.Unmarshal([]byte(val), &post); err != nil {
				log.Printf("failed to unmarshal post from key %s: %v", key, err)
				continue
			}
...
func (cache *redisCache) GetAll() []*Post {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
