...
func (cache *redisCache) Set(key string, value *user.UserCache) {
...
func (cache *redisCache) Get(key string) *user.UserCache {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (cache *redisCache) GetAll() []*user.UserCache {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
